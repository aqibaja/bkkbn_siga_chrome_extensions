 ...
 */
  ...
  async function bukaDanPilihPadaDropdown(control, targetTextRaw) {
    if (!control) return console.error("❌ Dropdown tidak ditemukan");
    const targetText = targetTextRaw.trim().replace(/\u2013|\u2014/g, '-').toLowerCase();
    control.scrollIntoView();
    control.focus();
    control.dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "ArrowDown", code: "ArrowDown" }));
    ...
    while (tries < maxTries) {
      const opsi = [...document.querySelectorAll('.css-yt9ioa-option, .css-1n7v3ny-option, .css-9gakcf-option')].find(el => {
        // Pastikan opsi adalah child dari control yang diinginkan
        return el.textContent.trim().replace(/\u2013|\u2014/g, '-').toLowerCase() === targetText
          && control.contains(el.closest('.css-1n7v3ny-option, .css-yt9ioa-option, .css-9gakcf-option'));
      });
      if (opsi) {
        opsi.click();
        console.log(`✅ Berhasil pilih "${targetTextRaw}" pada dropdown label yang tepat`);
        return;
      }
      await wait(100);
      tries++;
  ...
  if (desa) {
    await wait(1000);
    await bukaDanPilihDenganKeyboard(4, desa);         // Desa (tahunan)
  }
  if (rw) {
    await wait(1000);
    await bukaDanPilihDenganKeyboard(5, rw);           // RW (tahunan)
  }
  if (sasaran) {
    await wait(1000);
    await bukaDanPilihDenganKeyboard(6); // Sasaran (tahunan)
  }
